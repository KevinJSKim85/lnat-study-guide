...
(function (root) {
  ...
  function renderLearn() {
    ...
    if (lessons[0]) openLesson(lessons[0].id, { focus: false });
  }
  ...
  function openLesson(lessonId, opts) {
    ...
    if (progressStore) progressStore.recordLessonView(lessonId);
  ...
  /* ====================================================================== *
   * MOCKS — list 5, run timed (Section A then Section B), score report     *
   * ====================================================================== */
  ...
  function renderMockList() {
    ...
    mocks.forEach(function (mock) {
      ...
      var card = E("div", { class: "mock-card" }, [
        E("div", { class: "mock-card-main" }, [
      ...
      var btn = E("button", { type: "button", class: "btn btn-primary", text: "Start mock" });
  ...
  function startSectionA() {
    ...
    mockState.resolved.questions.forEach(function (q, i) {
      var block = E("div", { class: "qb-item" });
  ...
  function finishSectionA() {
    ...
    var reviewNote = E("p", { class: "muted", text: "Section A is locked. Scroll up to review every question with its rationale and worked solution. When ready, begin Section B." });
    reportHost.appendChild(reviewNote);

    var toB = E("button", { type: "button", class: "btn btn-primary", text: "Begin Section B (essay)" });
  ...
  function startSectionB() {
    ...
    // hide Section A questions to focus the writer (kept in DOM history via report link)
  ...
  function wireEssayPractice() {
    ...
    clear(host);
    var DATA = root.LNAT_DATA;
    host.appendChild(buildEssayWriter(DATA.prompts || [], { sectionLabel: "Practice", standalone: true }));
  }
  ...
  function buildEssayWriter(prompts, opts) {
    ...
    function finish() {
      ...
      checklistMount.appendChild(buildChecklistView(n));
  ...
  function buildChecklistView(wordCount) {
    var DATA = root.LNAT_DATA;
  ...
  function renderProgress() {
    ...
    if (byTypeEl) {
      ...
      if (!typesWithData.length) {
        ...
        var tb2 = E("tbody");
        typesWithData.forEach(function (t) {
          var s = stats[t];
          var pct = Math.round((s.correct / s.seen) * 100);
          var row = E("tr", {}, [
            E("td", { text: TYPE_LABELS[t] || t }),
            E("td", { text: s.correct + " / " + s.seen }),
            E("td", {}, [accuracyBar(pct)])
          ]);
          tb2.appendChild(row);
        });
        t2.appendChild(tb2);
